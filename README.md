...
- **[DeepAnalyze](https://github.com/ruc-datalab/DeepAnalyze)** — RUC-DataLab 开源的自主数据科学 Agent（[论文](https://arxiv.org/abs/2510.16872) / [模型](https://huggingface.co/RUC-DataLab/DeepAnalyze-8B)），自动完成数据探索、代码执行、可视化与报告生成

- **[WrenAI](https://github.com/Canner/WrenAI)** — Canner 开源的 GenBI 引擎，提供受治理的文本到 SQL 与语义层（MDL）。本仓库以 **`WrenAI/`** **子目录**形式纳入上游源码（含 `core/` SDK、`docs/`、`sdk/`、`skills/`、`examples/v5-jaffle/`、`evals/` 等），方便直接引用、查阅与本地构建 wren CLI

- **本仓库的核心工作区** — `DeepAnalyze/demo/chat_v2/`，把 DeepAnalyze 与 WrenAI 桥接成一个开箱即用的对话式数据分析应用（用户登录、会话隔离、语义层自动构建、代码沙箱执行、模型配置持久化等）

数据源：会话内上传的 CSV / Excel / SQLite 等文件，经自动构建的 DuckDB + 动态 MDL 进入 WrenAI 语义层；模型可通过注入的 `wren_query()` 函数对上传数据进行受治理 SQL 查询。
